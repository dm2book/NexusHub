  ...
  return (
    <div className="mb-10">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-extrabold text-slate-900">Bundle &amp; save</h2>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {bundles.map((b) => (
          <div key={b.id} className="relative bg-white rounded-2xl border border-slate-200/70 shadow-sm p-5 fm-lift overflow-hidden">
            <span className="absolute top-4 right-4 text-xs font-bold text-white bg-gradient-to-r from-violet-600 to-fuchsia-500 rounded-full px-2.5 py-1">Save {b.discountPercent}%</span>
            ...
            <p className="text-slate-400 text-sm mt-0.5 line-clamp-2">{b.description || b.products.map((p) => p.name).join(' + ')}</p>
            ...
            <button onClick={() => addBundle(b)} className="btn-primary w-full mt-4 py-2.5">
              <Plus size={16} /> Add bundle <ArrowRight size={15} className="fm-nudge" />
            </button>
          </div>
        ))}
      </div>
