    ...
    <section className="card mb-6">
      <header className="flex items-start justify-between gap-4 px-5 py-4 border-b border-white/5">
        <div>
          ...
          {subtitle && <p className="text-[13px] text-slate-400 mt-0.5 max-w-2xl">{subtitle}</p>}
        </div>
        {right}
      </header>
...
const STATUS_STYLE = {
  ...
  disabled: 'bg-white/5 text-slate-400 border-white/10',
  recommended: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/25',
  requires_review: 'bg-amber-500/10 text-amber-300 border-amber-500/25',
  approved: 'bg-violet-500/15 text-violet-300 border-violet-500/30',
  published: 'bg-sky-500/10 text-sky-300 border-sky-500/25',
  rejected: 'bg-white/5 text-slate-400 border-white/10',
};
const Pill = ({ status, children }) => (
  <span className={`text-[11px] font-semibold rounded-full px-2 py-0.5 border ${STATUS_STYLE[status] || 'bg-white/5 text-slate-400 border-white/10'}`}>
...
export default function AdminMarket() {
  ...
  if (!sources) {
    return <div className="p-8 text-slate-400 flex items-center gap-2"><Loader2 className="animate-spin" size={16} /> Loading market data…</div>;
  }
    ...
    <div className="max-w-[1400px] mx-auto px-4 lg:px-8 py-6">
      ...
      <Section
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {sources.map((s) => (
            <div key={s.key} className="border border-white/10 rounded-xl p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-slate-200 text-[14px] flex items-center gap-2">
                    {s.status === 'available' ? <ShieldCheck size={15} className="text-emerald-300" />
                      : <ShieldAlert size={15} className="text-amber-600" />}
                    {s.label}
                  </p>
                  <p className="text-[12.5px] text-slate-400 mt-1">{s.legalBasis}</p>
                  <p className="text-[12.5px] text-slate-300 mt-1.5">{s.statusReason}</p>
                </div>
              ...
              <div className="flex flex-wrap gap-3 mt-2.5 text-[11.5px] text-slate-400">
                {s.termsUrl && <a href={s.termsUrl} target="_blank" rel="noreferrer noopener"
                  className="hover:text-violet-400 inline-flex items-center gap-1">terms <ExternalLink size={11} /></a>}
      ...
      <Section title="Record a price you saw"
        ...
        <form onSubmit={submitObservation} className="grid sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
          <label className="lg:col-span-2 text-[12.5px] font-semibold text-slate-300">Listing title
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
              placeholder="EA FC Points 1050 PS5 EU"
              className="mt-1 w-full h-10 rounded-xl border border-white/10 px-3 text-sm font-normal" />
          </label>
          <label className="text-[12.5px] font-semibold text-slate-300">Price
            <input required value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })}
              inputMode="decimal" placeholder="12.49"
              className="mt-1 w-full h-10 rounded-xl border border-white/10 px-3 text-sm font-normal" />
          </label>
          <label className="text-[12.5px] font-semibold text-slate-300">Currency
            <input required value={form.currency} maxLength={3}
              onChange={(e) => setForm({ ...form, currency: e.target.value.toUpperCase() })}
              className="mt-1 w-full h-10 rounded-xl border border-white/10 px-3 text-sm font-normal uppercase" />
          </label>
          <label className="lg:col-span-2 text-[12.5px] font-semibold text-slate-300">Source URL
          ...
          <div className="lg:col-span-6 flex items-center gap-3">
            <select value={form.availability} onChange={(e) => setForm({ ...form, availability: e.target.value })}
              className="h-10 rounded-xl border border-white/10 px-3 text-sm">
      ...
      <Section title="Product discovery"
        ...
        right={<button onClick={() => act('discover', async () => {
          ...
          className="text-sm font-semibold text-violet-300 hover:text-violet-800 inline-flex items-center gap-1.5">
            ...
            return (
              <button key={b.key} onClick={() => setOpen(b.key)}
                className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-[13px] font-semibold border transition
                  ${open === b.key ? 'bg-violet-500/15 border-violet-500/40 text-violet-300' : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}>
        ...
        <p className="text-[12.5px] text-slate-400 mb-3">{BUCKETS.find((b) => b.key === open)?.hint}</p>
            ...
            <tbody>
              {(discovery?.[open] || []).map((c) => (
                <tr key={c.id} className="border-t border-white/5">
                  <td className="py-2.5 pr-3 font-semibold text-slate-200">{c.title}</td>
                  <td className="pr-3 text-slate-400 font-mono text-[11px]">{c.canonical_key}</td>
                  <td className="pr-3 text-slate-300">{c.reason}</td>
                  <td className="pr-3 text-slate-400 whitespace-nowrap">
                  ...
                  <td className="text-right whitespace-nowrap">
                    <button onClick={() => act(`ap-${c.id}`, async () => {
                      ...
                      className="text-emerald-300 hover:bg-emerald-500/10 rounded-lg px-2 py-1 font-semibold">
                    ...
                    {c.status === 'approved' && (
                      <button onClick={() => act(`cr-${c.id}`, async () => {
                        ...
                        className="text-violet-300 hover:bg-violet-500/10 rounded-lg px-2 py-1 font-semibold ml-1">
                    ...
                    <button onClick={() => act(`rj-${c.id}`, async () => {
                      ...
                      className="text-slate-400 hover:bg-white/10 rounded-lg px-2 py-1 font-semibold ml-1">
            ...
            <tbody>
              {recs.map((r) => (
                <tr key={r.id} className="border-t border-white/5 align-top">
                  <td className="py-2.5 pr-3">
                    ...
                    {!!r.blockers.length && (
                      <ul className="mt-1 space-y-0.5">
                        {r.blockers.map((b) => (
                          <li key={b.code} className="text-[11.5px] text-amber-300 flex items-start gap-1">
                  ...
                  <td className="pr-3 text-slate-400">{fmt(r.official_cents)}</td>
                  <td className="pr-3">
                    {r.competitor_count}
                  ...
                  <td className="pr-3 font-bold text-white">{fmt(r.recommended_cents)}</td>
                  <td className="pr-3">{pct(r.margin_pct)}</td>
                  <td className="pr-3">{fmt(r.profit_cents)}</td>
                  <td className="pr-3 whitespace-nowrap">
                    <span className={r.stale ? 'text-amber-300 font-semibold' : 'text-slate-400'}>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div>
                        <button onClick={() => act(`ra-${r.id}`, async () => {
                          ...
                          className="text-emerald-300 hover:bg-emerald-500/10 rounded-lg px-2 py-1 font-semibold disabled:opacity-40">
                          Approve
                        </button>
                        <button onClick={() => act(`rr-${r.id}`, async () => {
                          ...
                          className="text-slate-400 hover:bg-white/10 rounded-lg px-2 py-1 font-semibold disabled:opacity-40">
                          Reject
                        </button>
                        ...
                        <button onClick={() => act(`rp-${r.id}`, async () => {
                          ...
                          className="text-violet-300 hover:bg-violet-500/10 rounded-lg px-2 py-1 font-semibold disabled:opacity-40">
                          Publish
                        </button>
                      </div>
                    </div>
            ...
            <tbody>
              {history.map((h) => (
                <tr key={h.id} className="border-t border-white/5">
                  <td className="py-2 pr-3 text-slate-400 whitespace-nowrap">{ago(h.created_at)}</td>
                  ...
                  <td className="pr-3 text-slate-300">{h.actor || '—'}</td>
                  <td className="text-slate-400">{h.reason}</td>
                </tr>
              ))}
              {!history.length && (
