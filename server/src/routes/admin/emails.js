...
router.get('/', requirePermission('emails.manage'), (_req, res) => {
  res.json({ templates: all('SELECT * FROM email_templates ORDER BY name') });
});

router.get('/log', requirePermission('emails.manage'), (_req, res) => {
  res.json({ log: all('SELECT * FROM email_log ORDER BY created_at DESC LIMIT 100') });
});

router.get('/:id', requirePermission('emails.manage'), (req, res) => {
  const t = get('SELECT * FROM email_templates WHERE id=@id', { id: req.params.id });
...
router.put('/:id', requirePermission('emails.manage'), asyncHandler(async (req, res) => {
  ...
  run(`UPDATE email_templates SET name=@name, subject=@subj, body_html=@body,
  ...
  audit({ actor: req.user, action: 'email.template_update', targetType: 'email_template',
    targetId: req.params.id, req });
  res.json({ template: get('SELECT * FROM email_templates WHERE id=@id', { id: req.params.id }) });
}));
