...
export function listProducts({ activeOnly = false } = {}) {
  ...
  return all(`SELECT * FROM products ${clause} ORDER BY created_at DESC`).map(hydrate);
}

export function getProduct(id) {
  return hydrate(get('SELECT * FROM products WHERE id = @id', { id }));
}
...
export function createProduct(p = {}) {
  ...
  const id = newId('prd');
  const at = nowIso();
  run(`INSERT INTO products (id, sku, name, category, description, price, currency, kind, stock, active, metadata, created_at, updated_at)
...
export function updateProduct(id, patch = {}) {
  ...
  run(`UPDATE products SET name=@name, sku=@sku, category=@cat, description=@desc,
